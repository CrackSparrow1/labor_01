...
class AdatbazisKezelo:
    def __init__(self, ablak):
        ...
        self.ablak.geometry("660x320")  # Ablak méretének beállítása
        ...
        self.ablak.configure(bg="#f0f0f0")  # Háttérszín beállítása

        self.rekordok_lista = tk.Listbox(self.ablak, bg="white", selectbackground="blue", font=("Helvetica", 12))  # Lista háttérszín és kijelölési szín beállítása, és betűméret növelése
        self.rekordok_lista.pack(fill="both", expand=True, padx=5, pady=5)  # A lista méretének beállítása

        self.torol_gomb = tk.Button(self.ablak, text="Rekord törlése", command=self.torol_rekord, bg="#FF5733", fg="white", font=("Helvetica", 12))  # Gomb formázása és betűméret növelése
        self.torol_gomb.pack(side="bottom", pady=5)  # A "Rekord törlése" gomb alulra helyezése

        self.uj_rekord_gomb = tk.Button(self.ablak, text="Új rekord", command=self.uj_rekord, bg="#4CAF50", fg="white", font=("Helvetica", 12))  # Gomb formázása és betűméret növelése
        self.uj_rekord_gomb.pack(side="bottom", pady=5)  # Az "Új rekord" gomb alulra helyezése
    ...
    def uj_rekord(self):
        ...
        nev_cimke = tk.Label(rekord_dialog, text="Név:", font=("Helvetica", 12))  # Betűméret növelése
        nev_cimke.pack()

        nev_mezo = tk.Entry(rekord_dialog)
        nev_mezo.pack()

        email_cimke = tk.Label(rekord_dialog, text="E-mail:", font=("Helvetica", 12))  # Betűméret növelése
        email_cimke.pack()

        email_mezo = tk.Entry(rekord_dialog)
        email_mezo.pack()

        mentes_gomb = tk.Button(rekord_dialog, text="Mentés", command=lambda: self.mentes_es_frisstes(nev_mezo.get(), email_mezo.get(), rekord_dialog), font=("Helvetica", 12))  # Gomb formázása és betűméret növelése
        mentes_gomb.pack()
